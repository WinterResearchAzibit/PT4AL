...
rand_seed = 90
makeDeterministic(rand_seed)

open('./rotation_loss.txt', 'w').close()
open('./loss/batch_0.txt', 'w').close()
...
# net = torch.load('./checkpoint/rotation.pth')
net = torch.nn.DataParallel(net)
net.eval()
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, inputs1, inputs2, inputs3, targets, targets1, targets2, targets3, path) in enumerate(testloader):
            ...
            print(f"Input: {inputs.mean()} and output: {outputs.mean()}")
            ...
            for idx in range(len(loss_list)):
                s = str(float(loss_list[idx])) + '_' + str(path[idx]) + '\n'
